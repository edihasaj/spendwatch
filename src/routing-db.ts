...
CREATE TABLE IF NOT EXISTS routing_runs (
  ...
  verification_json TEXT, output TEXT, error TEXT
);
CREATE TABLE IF NOT EXISTS routing_attempts (
  ...
  verification_json TEXT, usage_json TEXT, error TEXT
);
...
export class RoutingStore {
  ...
  start(plan: RoutePlan, shadow: boolean): number {
    const result = this.db.prepare(`INSERT INTO routing_runs
      (task_id,policy_version,task,repo,planned_model,planned_effort,shadow,status,started_at)
      VALUES (?,?,?,?,?,?,?,'running',?)`).run(
        plan.taskId, plan.policyVersion, plan.task, plan.repo, plan.decision.model,
        plan.decision.effort, shadow ? 1 : 0, new Date().toISOString(),
      );
    return Number(result.lastInsertRowid);
  }
  attempt(runId: number, record: AttemptRecord): void {
    this.db.prepare(`INSERT INTO routing_attempts
      (run_id,attempt,provider,model,effort,started_at,duration_ms,exit_code,passed,verification_json,usage_json,error)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).run(
        ...
        JSON.stringify(record.verification), JSON.stringify(record.result.usage ?? null), record.result.error ?? null,
      );
  }
  finish(runId: number, status: "succeeded" | "failed", records: AttemptRecord[], output: string, error?: string): void {
    ...
    this.db.prepare(`UPDATE routing_runs SET actual_model=?,provider=?,status=?,finished_at=?,duration_ms=?,attempts=?,verification_json=?,output=?,error=? WHERE id=?`).run(
      last?.model ?? null, last?.result.provider ?? null, status, new Date().toISOString(), duration, records.length,
      JSON.stringify(last?.verification ?? []), output.slice(0, 20_000), error ?? null, runId,
    );
  }
